...
class APIConnectors:
    ...
    def fetch_youtube_trends(self, query: str = "trending"):
        ...
        if not self.youtube_key:
            print("⚠️ [YouTube] API Key is missing. Check your environment variables.")
            return []
    ...
    def fetch_active_realtime_trends_from_db(self):
        """
        Fetches the pre-calculated algorithmic trends from Supabase for production consistency.
        Routes through DataRouter to ensure it pulls from 'trending_top_20'.
        """
        # DataRouter will automatically route this to Supabase if 'trending_top_20' is in SUPABASE_TABLES
        from data_router import router
        
        try:
            # Table 'active_realtime_trends' exists on Supabase and contains live trends
            query = """
            ...
            if not rows:
                print("⚠️ [APIConnectors] No trends found in active_realtime_trends. Check Supabase sync.")
                return []
            ...
            print(f"❌ Error fetching active trends from DataRouter: {e}")
            return []
    ...
    @cache.cached(source="NaverDatalab", expiry=3600)
    def fetch_naver_datalab_search(self, keywords: List[str]):
        """
        Fetches relative search volume from Naver Datalab.
        """
        if self.mode == "MOCK":
            return {k: [{"period": "2024-05-20", "ratio": 100}] for k in keywords}
        ...
        # Prepare body (Groups)
        groups = []
        for word in keywords[:5]: # Max 5 groups per call
            groups.append({
                "groupName": word,
                "keywords": [word]
            })
        ...
        try:
            ...
            print(f"⚠️ [Naver Datalab] Exception: {e}")
            return {}

    @cache.cached(source="YouTubeLive", expiry=600)
    def fetch_youtube_live(self, query: str = "뉴스"):
        """
        Fetches currently live broadcasts to determine 'real-time' interest.
        """
        if self.mode == "MOCK":
            return [{"title": "[LIVE] 속보: AI 혁명 시작", "viewers": 15000}]
        ...
        try:
            ...
            if response.status_code == 200:
                items = response.json().get("items", [])
                # Could fetch statistics for viewer count (requires extra call/cost)
                # For now, just existence of many live streams is a signal
                return items
            return []
        except Exception as e:
            print(f"⚠️ [YouTube Live] Exception: {e}")
            return []

    def fetch_naver_blog_count(self, query):
        """
        Headless approach to get rough 'posting velocity'.
        Uses the search API check how many items in last 24h.
        """
        # This is an estimation using the 'sort=date' feature of Search API
        if self.mode == "MOCK":
            return 50 # 50 posts per hour
