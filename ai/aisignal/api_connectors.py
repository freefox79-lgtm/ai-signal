...
class APIConnectors:
    ...
    def fetch_active_realtime_trends_from_db(self):
        """
        Fetches the pre-calculated algorithmic trends from the database.
        These are generated by scripts/trend_collector.py (Z-Score + Velocity).
        """
        if self.mode == "MOCK":
            return [
                {
                    "rank": 1, "keyword": "AI Algorithmic Search", 
                    "avg_score": 98.5, "related_insight": "System Active (Mock)", 
                    "status": "NEW", "link": "#"
                }
            ]
